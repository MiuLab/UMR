...
class DenseRetriever(object):
    ...
    def index_encoded_data(self, vector_files: List[str], buffer_size: int = 50000):
        ...
        buffer = []
        for i, item in enumerate(tqdm(iterate_encoded_files(vector_files))):
    ...
    def get_top_docs(
        ...
        """
        time0 = time.time()
        results = self.index.search_knn(query_vectors, top_docs, batch_size=batch_size)
...
def parse_qa_jsonlines_file(location, add_lang=False) -> Iterator[Tuple[str, str, List[str], str]]:
    ...
    for row in data:
        ...
        answers = row["answers"] if "answers" in row else [""]
        q_id = row["id"]
        lang = row["lang"]
        if add_lang is True:
...
def save_results(
    ...
    with open("{}_xor_retrieve_results.json".format(out_file.replace(".json", "")), 'w') as outfile:
...
def main(args):
    if args.model_file and os.path.exists(args.model_file):
        ...
        raise ValueError("args.model_file is not found in local file system. If you want to load a pretrained model \
            from Huggingface Hub, set args.pretrained_model_cfg instead")
    ...
    if args.hnsw_index:
        index = DenseHNSWFlatIndexer(vector_size, buffer_size=args.index_buffer)
        index_buffer_sz = -1  # encode all at once
    else:
        index = DenseFlatIndexer(vector_size, buffer_size=args.index_buffer, use_gpu=args.use_gpu)
    ...
    top_ids_and_scores = retriever.get_top_docs(
        questions_tensor.numpy(), args.n_docs, batch_size=args.search_batch_size)
    ...
    if args.out_file:
        save_results(
            all_passages, questions, q_ids, question_answers, question_languages,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--search_batch_size', type=int, default=1024, help="Batch size for kNN search")
    parser.add_argument('--index_buffer', type=int, default=50000,
                        help="Temporal memory data buffer size (in samples) for indexer")
    parser.add_argument("--hnsw_index", action='store_true', help='If enabled, use inference time efficient HNSW index')
    parser.add_argument("--use_gpu", action='store_true', help='If enabled, use gpu for indexing and search')
    ...
    if args.encode_q_and_save and args.q_encoding_path is None:
        raise ValueError("Requires q_encoding_path when encoding question")
