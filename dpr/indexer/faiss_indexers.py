...
class DenseIndexer(object):
    ...
    def search_knn(
        self, query_vectors: np.array, top_docs: int, batch_size: int = 100
...
class DenseFlatIndexer(DenseIndexer):
    ...
    def __init__(self, vector_sz: int, buffer_size: int = 50000, use_gpu: bool = False):
    ...
    def search_knn(
        ...
        result = []
        for batch_start in tqdm(range(0, len(query_vectors), batch_size)):
            scores, indexes = self.index.search(query_vectors[batch_start:batch_start + batch_size], top_docs)
...
class DenseHNSWFlatIndexer(DenseIndexer):
    ...
    def __init__(
        self, vector_sz: int, buffer_size: int = 50000, store_n: int = 512,
    ...
    def search_knn(
        ...
        result = []
        for batch_start in tqdm(range(0, len(query_vectors), batch_size)):
            scores, indexes = self.index.search(query_nhsw_vectors[batch_start:batch_start + batch_size], top_docs)
            # convert to external ids
            db_ids = [[self.index_id_to_db_id[i] for i in query_top_idxs] for query_top_idxs in indexes]
            result.extend([(db_ids[i], scores[i]) for i in range(len(db_ids))])
        return result
