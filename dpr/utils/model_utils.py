...
def setup_for_distributed_mode(model: nn.Module, optimizer: torch.optim.Optimizer, device: object, n_gpu: int = 1,
                               local_rank: int = -1,
                               fp16: bool = False,
                               fp16_opt_level: str = "O1") -> Tuple[nn.Module, torch.optim.Optimizer]:
    model.to(device)
    if n_gpu > 1:
...
def load_states_from_checkpoint(model_file: str) -> CheckpointState:
    ...
    if "ctx_model.embeddings.position_ids" not in state_dict["model_dict"]:
        state_dict["model_dict"]["ctx_model.embeddings.position_ids"] = torch.arange(0, 512).expand(1, -1).long()
    if "question_model.embeddings.position_ids" not in state_dict["model_dict"]:
        state_dict["model_dict"]["question_model.embeddings.position_ids"] = torch.arange(0, 512).expand(1, -1).long()
